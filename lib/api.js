...
let mtbcReward = require('./reward.js');
	...
	async.parallel({
		...
		config: function (callback) {
			...
			callback(null, {
				...
				// MateableCoin specific
				workerSeparator: '+',
				algorithms: ['yescryptr8'],
...
function getLastBlockData (callback) {
	rpc.call('getbestblockhash', [], function (error, hash) {
		...
		rpc.call('getblockheader', [hash], function (error, header) {
			...
			let hashes = header.difficulty || 0;
			if (header.bits) {
				let target = require('./blockBuilder.js').compactToTarget(parseInt(header.bits, 16));
				if (target > 0n) hashes = Number((1n << 256n) / (target + 1n));
			}
			callback(null, {
				difficulty: hashes,
				height: header.height,
				timestamp: header.time,
				reward: mtbcReward.minerReward(header.height),
				hash: header.hash
			});
		});
	});
 ...
 **/
/**
 * Shorten an address for the public top 10: the beginning and the end.
 **/
