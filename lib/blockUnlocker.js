/**
 * Block unlocker for MateableCoin (adapted from cryptonote-nodejs-pool, GPL-2.0).
 *
 ...
 * once the chain is `depth` blocks past it (the node's own coinbase maturity, consensus.nCoinbaseMaturity, is 59 blocks; keep depth a bit above):
...
// Developer donation: {"<MateableCoin address>": percent of the block reward}. It is paid like any other balance (same minimum,
...
let donations = utils.donationTable(unlockerConfig, function (address) {
	log('error', logSystem, 'Donation entry %s ignored: it needs a valid MateableCoin address and a percent above 0 up to 10', [address]);
});
